/**
 * BFF proxy: browser -> same-origin /api/content/* -> FastAPI /content/* with
 * the session's bearer token attached HERE, server-side (tokens never touch
 * JS - D10 non-negotiable). Only the backend's /content prefix is reachable
 * through this route by construction.
 *
 * A-U3 W1: this exists for /content/bookmarks, which is PRIVATE (a reader's
 * own shelf). The public reads under /content — the feed and item pages —
 * are fetched server-side in lib/content.ts, never through here, so nothing
 * that needs no session pays for one.
 */
...
async function forward(
  ...
  const url = new URL(
    `${API}/content/${path.map(encodeURIComponent).join("/")}`,
  );
