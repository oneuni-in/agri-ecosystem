/**
 * BFF proxy: browser -> same-origin /api/market/* -> FastAPI /market/* with
 * the session's bearer token attached HERE, server-side (tokens never touch
 * JS - D10 non-negotiable). Only the backend's /market prefix is reachable
 * through this route by construction.
 *
 * A-U2 AG-A16: this exists for /market/alerts, which is PRIVATE (a user's
 * own subscriptions). The public reads under /market — today, commodities
 * — are fetched server-side in lib/, never through here, so nothing that
 * needs no session pays for one.
 */
...
async function forward(
  ...
  const url = new URL(`${API}/market/${path.map(encodeURIComponent).join("/")}`);
