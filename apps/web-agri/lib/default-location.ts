/**
 * Where a visitor with no location of their own starts (ported from
 * web-milk's `lib/default-location.ts` — the proven U1 pattern).
 *
 ...
 * label and the server renders every location-bound section from this same
 * pincode, so the page can never be internally inconsistent.
 *
 * Its own module because BOTH sides need it: `lib/home.ts` is server-only
 * (it holds `API_BASE_URL` and server `fetch`), and importing that into a
 * client island would drag server code into the browser bundle.
 *
