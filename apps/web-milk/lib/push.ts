/**
 * D28 web-push subscription, shared by every surface that offers alerts:
 * §10a's price-alert card on the home and the device toggle on
 * /notifications. Two copies of a subscribe/unsubscribe flow drift — one of
 * them ends up leaving orphan browser subscriptions behind when the server
 * call fails, which is exactly the bug this module's single `subscribePush`
 * is written to avoid.
 *
