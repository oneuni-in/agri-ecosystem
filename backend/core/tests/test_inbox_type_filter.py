...
async def test_type_filter_returns_only_matching(
...
async def test_no_filter_returns_all(api: tuple[httpx.AsyncClient, AsyncSession]) -> None:
    http, session = api
    owner = uuid.uuid4()
    business_id = await _seeded_inbox(session, owner)
...
async def test_bogus_type_422(api: tuple[httpx.AsyncClient, AsyncSession]) -> None:
    http, session = api
    owner = uuid.uuid4()
    business_id = await _seeded_inbox(session, owner)
    response = await http.get(
