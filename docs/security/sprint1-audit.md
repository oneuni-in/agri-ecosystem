...
**Informational note (documented, not fixed in D14):** `shared/events.py::publish`
does a bare `XADD` with no `MAXLEN`/`XTRIM`, so the `identity` stream grows
unbounded. `coins/worker.py` (`GROUP="coins"`, `NAME="coins-worker-1"`) is a
single-replica, serial consumer of this same stream, processing
`user.registered` and `profile.completed` (economically important awards)
off the same head-of-line ordering as `identity.session_resumed` (added
Task 8, `0849b0b`) — a high volume of session-resumed traffic shares queue
position with those awards. Not a correctness bug (every award is
idempotent via `rules.deterministic_key` + the `UNIQUE(idempotency_key)`
constraint, so no data loss or double-award is possible), just a
throughput/latency scaling gap, in the same spirit as `coins/worker.py`'s
own "KNOWN BUS LIMITATION (D12)" docstring comment about missing
idle-redelivery/XAUTOCLAIM. Recorded as a fast-follow; adding a `MAXLEN`
cap to `shared/events.py::publish` is out of scope here since it would
affect every event producer in the repo, not just this call site.
...
|---|---|---|---|---|
| Critical | OTP | `backend/core/modules/identity/otp_service.py:122-146` | `verify_otp` reads `attempts` with a plain `SELECT` (no `FOR UPDATE`, no optimistic lock) then writes `attempts=<python-computed literal>` on flush. Concurrent verify requests against the same active OTP each read the same stale count and each independently write `old+1`, a genuine lost-update — `OTP_MAX_ATTEMPTS=3` never trips under concurrent guessing, so the 6-digit code can be brute-forced with far more than 3 tries inside its TTL. | fixed — confirmed by independent re-read of `otp_service.py`, `shared/db.py`, and `OtpRequest`'s mapper (no locking/version column anywhere in the path; Postgres READ COMMITTED row-locking on the blind `UPDATE` does not prevent the second writer from clobbering with the same stale value). **Fixed in `ccebef9`**: `.with_for_update()` added to the row `SELECT` in `verify_otp`, closing the race via Postgres row-level locking (concurrent callers now serialize on the row instead of both reading the same stale `attempts`). Proven by a real-concurrent-transaction test (`backend/core/tests/test_otp_service.py::test_concurrent_wrong_guesses_do_not_lose_attempts` and `::test_concurrent_wrong_guesses_beyond_cap_burn_the_code`, separate `AsyncSession`/engine connections per `asyncio.gather`-driven task, `test_coins_storm.py`'s pattern): against the pre-fix code the same test asserts fail (3 or 5 concurrent wrong guesses collapse to a persisted `attempts=1`, never burning); against the fix, the persisted count lands exactly on the cap and the code burns. |
| High | OTP | `backend/core/modules/identity/otp_service.py:78-102`, `backend/core/modules/identity/otp_throttle.py:76-97,100-126` | The resend-cooldown key (`otp:cd:{phone}`) is only written at the end of `register_issue`, after the DB update/insert/flush; `assert_issue_allowed`'s cooldown check is a plain TTL read with no atomic claim-the-slot (`SET NX`) beforehand. Concurrent `/auth/otp/request` calls for the same phone all pass the cooldown gate before any of them writes it, letting an attacker burn a victim's daily OTP quota (and send multiple SMS) in one synchronous burst instead of the intended 30s/60s/300s escalating pacing. | fixed — confirmed by independent trace of the exact Redis/DB call order (`assert_issue_allowed` and `register_issue` were two non-atomic operations separated by an awaited DB flush, with no `SET NX EX` anywhere in the path; only the atomic daily cap (5/day) bounded total volume, not per-request pacing). **Fixed in `1fcfc28`**: `assert_issue_allowed` now atomically claims the cooldown key with `SET key val NX EX RESEND_COOLDOWNS_SECONDS[0]` before any other check; only one of N concurrent callers for the same phone can win the claim, the rest are rejected immediately with `OtpRateLimited`. `register_issue`'s existing unconditional `SET` (no `NX`) still runs afterward and overwrites the provisional claim with the real escalated-TTL cooldown once issuance succeeds. Proven by a real-concurrency test (`backend/core/tests/test_otp_throttle.py::test_concurrent_issue_requests_only_one_claims_cooldown`, `asyncio.gather` over 5 concurrent calls against the real test Redis): against the pre-fix code all 5 calls pass the cooldown check concurrently; against the fix, exactly 1 succeeds and the other 4 raise `OtpRateLimited` from the cooldown claim itself. |
...
| High | RBAC | `backend/core/modules/identity/admin_router.py:328-340` | `reactivate_user` (permission `users.suspend`, held by the `staff` role per `0008_identity_seed_roles.py:50`) never calls the `_guard_suspend_target` check that `suspend_user` uses at line 310. A `staff`-role principal can therefore unilaterally reactivate a `super_admin` account that was suspended for incident response, with no super_admin-level authorization on the reactivate side — breaking the exact escalation-guard symmetry the suspend side enforces. Untested: `test_admin_router.py`'s only related test covers suspend, not reactivate, against a super_admin target. | fixed — confirmed by independent re-read that `staff` genuinely held `users.suspend` (seed migration), `reactivate_user` called no guard of any kind, and `require_permission`/`SecureRouter` have no built-in role-hierarchy logic that would otherwise close the gap. **Fixed in `6351d8d`**: `reactivate_user` now calls `await _guard_suspend_target(session, user, principal)` right after resolving the target user, identical to `suspend_user`'s call shape/placement. Proven by `backend/core/tests/test_admin_router.py::test_cannot_reactivate_super_admin_as_staff`: against the pre-fix code, a staff principal reactivating a suspended super_admin returns 200; against the fix, it returns 403 (`super_admin_required`), while a super_admin principal reactivating the same account still returns 200. |
...
| Informational | Coins | `backend/core/modules/coins/rules.py:32-46` | `daily_visit`'s `deterministic_key` takes a caller-supplied `day` argument rather than deriving it from a trusted server clock inside the function. **Update (D14 Task 8, `0849b0b`):** the rule now has a live caller — `coins/worker.py`'s `identity.session_resumed` handler computes `day = now.strftime("%Y-%m-%d")` from the event handler's own server-derived `now` parameter (never from the event payload or any client input) and passes it straight into `deterministic_key`. The server-derivation concern this row raised is satisfied at the only call site that exists today; the function's signature itself is unchanged (a future caller could still pass a client-supplied value), so this is downgraded from an open concern to a confirmed-satisfied observation rather than removed outright. | open |
| Critical | Audit/Notify | `backend/core/modules/coins/admin_router.py:231-240` | `adjust_confirm` (manual coin-balance adjustment, dual-confirm flow) calls `publish("audit", "coins.manual_adjust", {...})` — a bare Redis-stream `xadd` — instead of `shared.audit.audit()`. No consumer anywhere in the repo subscribes to a stream named `"audit"` (notify's worker reads `("identity","notify")`; coins' worker reads `"identity"` only), so the event is permanently orphaned. The admin's `reason_note` justification exists only as a short-TTL Redis key that is `getdel`'d earlier in the same handler and is not a column on `LedgerEntry` — it becomes unrecoverable the instant the handler returns. The module's own docstring claims this flow is "audit-logged"; it is not, in practice. | fixed — confirmed by independent re-read that `shared.audit` was never imported in this file, no consumer group existed on the `"audit"` stream repo-wide, and `LedgerEntry` had no free-text column; no reconciliation job or hidden bridge was found. **Fixed in `91c7dc6`**: `adjust_confirm` now calls `await audit(session, action="coins.manual_adjust", actor_user_id=uuid.UUID(admin_id), target_type="user", target_id=str(user_id), metadata={"delta": delta, "reason_note": intent["reason_note"]})` in the same session/transaction as `service.record_entry`, making the ledger write and its audit record atomic. `shared.audit` has no `modules.*` imports, so this does not violate the `modules.coins`/`modules.identity` independence contract (`lint-imports` still passes: 3 kept, 0 broken). Proven by `backend/core/tests/test_coins_admin_router.py::test_manual_adjust_writes_real_audit_entry_not_orphaned_stream`: against the pre-fix code, no `AuditEntry` row is written for `coins.manual_adjust`; against the fix, a real hash-chained row exists with the correct `actor_user_id`/`target_id`/`meta.delta`/`meta.reason_note`, and the `"audit"` Redis stream stays empty (`xlen == 0`). |
| High | Audit/Notify | `backend/core/modules/coins/admin_router.py:160-173` | `update_rule` (`PUT /admin/coins/rules/{code}`, lets a `super_admin` change a coin rule's amount/caps/active flag/validity window) has **no** audit call and **no** event publish of any kind — not even the (separately flagged) broken `publish("audit", ...)` pattern used elsewhere in this same file. A `super_admin` can silently change reward economics system-wide with zero trace beyond the overwritten row; `coins.rules` has no history table and the ledger's immutability trigger is scoped only to `ledger_entries`, not `rules`. | fixed — confirmed by independent re-read that no audit/publish call existed anywhere in the handler body, no router-level audit dependency was applied uniformly to `admin_router.py`, and no history/versioning table or trigger existed on `coins.rules`. **Fixed in `edd26af`**: `update_rule` now captures the acting admin's `user_id` from `_require_role` (previously discarded), snapshots the rule's pre-mutation field values for only the fields being changed, applies the mutation, `flush()`es, then calls `await audit(session, action="coins.rule_updated", actor_user_id=admin_id, target_type="coins_rule", target_id=code, metadata={"before": before, "after": changes})` in the same transaction — `changes` comes from `body.model_dump(mode="json", exclude_none=True)` so datetime fields (`valid_from`/`valid_to`) serialize to JSONB-safe ISO8601 strings rather than raw `datetime` objects. Proven by `backend/core/tests/test_coins_admin_router.py::test_rule_edit_writes_real_audit_entry_with_before_after`: against the pre-fix code, no `AuditEntry` row is written for `coins.rule_updated`; against the fix, a real hash-chained row exists with the correct `actor_user_id`/`target_type`/`target_id`, and `meta["before"]`/`meta["after"]` contain the correct old/new values for both a numeric field (`amount`) and datetime fields (`valid_from`/`valid_to`), which round-trip correctly through ISO8601 serialization. |
...
**Critical/High rollup (feeds Task 14):** 5 findings, all confirmed, all now fixed, zero open —
OTP verify-attempts lost-update race (Critical, **fixed in `ccebef9`**),
OTP issue-cooldown race (High, **fixed in `1fcfc28`**),
RBAC `reactivate_user` missing super-admin-target guard (High, **fixed in `6351d8d`**),
coins `adjust_confirm` audit-orphan / unrecoverable `reason_note` (Critical, **fixed in `91c7dc6`**),
coins `update_rule` zero audit trail (High, **fixed in `edd26af`**). None were refuted.
